  ...
  return html`
    <button class="device ${statusClass}" @click=${() => onHandleAction(device.entityId, 'more-info')}>
      ...
      <div class="control-row" style="gap:4px" @click=${(e: Event) => e.stopPropagation()}>
        ...
        <select class="filter-select" style="font-size:var(--sp-font-4xs);min-height:22px;min-width:44px;padding:0 14px 0 4px;background-size:8px;flex-shrink:0" @change=${(e: Event) => { e.stopPropagation(); doService('set_preset_mode', { preset_mode: (e.target as HTMLSelectElement).value }); }} @click=${(e: Event) => e.stopPropagation()}>
        ...
        <button class="media-volbtn" style="width:24px;height:24px;padding:0;flex-shrink:0" title=${t(language, 'fanOscillate')} @click=${(e: Event) => { e.stopPropagation(); doService('oscillate', { oscillating: !oscillating }); }}><ha-icon icon=${oscillating ? 'mdi:rotate-3d-variant' : 'mdi:rotate-360'} style="--mdc-icon-size:14px"></ha-icon></button>` : ''}
        ${isOn && currentDirection !== undefined ? html`
        <button class="media-volbtn" style="width:24px;height:24px;padding:0;flex-shrink:0" title=${t(language, 'fanDirection')} @click=${(e: Event) => { e.stopPropagation(); doService('set_direction', { direction: currentDirection === 'forward' ? 'reverse' : 'forward' }); }}><ha-icon icon=${currentDirection === 'reverse' ? 'mdi:reload' : 'mdi:swap-vertical'} style="--mdc-icon-size:14px"></ha-icon></button>` : ''}
