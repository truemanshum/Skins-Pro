    ...
    <button class="device ${statusClass}" @click=${() => onHandleAction(device.entityId, 'more-info')}>
      ...
      <div class="control-row" style="gap:2px" @click=${(e: Event) => e.stopPropagation()}>
        <div style="display:flex;align-items:center;gap:1px;flex-shrink:0">
          <button class="media-volbtn" style="width:28px;height:32px;padding:0" @click=${(e: Event) => { e.stopPropagation(); adjustTemp(-step); }}><ha-icon icon="mdi:minus" style="--mdc-icon-size:14px"></ha-icon></button>
          ...
          <button class="media-volbtn" style="width:28px;height:32px;padding:0" @click=${(e: Event) => { e.stopPropagation(); adjustTemp(step); }}><ha-icon icon="mdi:plus" style="--mdc-icon-size:14px"></ha-icon></button>
        </div>
