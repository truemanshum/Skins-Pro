...
Skins Pro 是一款社区 Lovelace 卡片，采用多皮肤架构。内置 **modern** 皮肤，并通过皮肤商店提供 **AEON**、**AEON_glass**、**visionOS**、**minecraft** 及社区投稿皮肤。自带中英文双语，从 HACS 安装后无需配置即可使用。
...
除了内置皮肤，还可以在卡片编辑器中直接下载社区皮肤：

![皮肤商店](screenshots/Advanced_Feature.png)

商店从 [store 分支](https://github.com/ha-china/Skins-Pro/tree/store) 通过 CDN 获取可用皮肤列表。点击**下载**时，卡片调用 [`skins-pro-hass`](https://github.com/ha-china/skins-pro-hass) 集成将皮肤包下载并解压到 HA 的 `www/` 目录——无需浏览器上传，无需 YAML 配置。

> 需要先安装 [skins-pro-hass 集成](https://github.com/ha-china/skins-pro-hass) 才能使用下载功能。
...
| modern（内置） | AEON（商店） | visionOS（商店） | minecraft（商店） |
|---|---|---|---|
