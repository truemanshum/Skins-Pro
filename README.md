...
- 🤖 Automations page<br>自动化页面
- ⚡ Energy dashboard (today vs yesterday)<br>能源面板（含昨日对比）
- 🛡️ Security page — cameras, locks, alarm control panel (auto-detected, click to arm/disarm)<br>安全页面 — 摄像头、门锁、安防面板（自动检测，点击可布撤防）
- 🎵 Media player card — album art, playback controls, volume bar<br>媒体播放器卡片 — 专辑封面、播放控制、音量条
- 📷 Camera snapshot on homepage<br>首页摄像头实时快照
- 🌡️ Environment sensors display<br>环境传感器展示
- 🌐 Auto CN/EN bilingual switching<br>中英文双语自动切换
- ↔️ Fullscreen Kiosk mode<br>全屏 Kiosk 模式
- 🖼️ Use HA area pictures as room backgrounds<br>使用 HA 区域图片作为房间背景
- 🎨 Custom background image upload<br>自定义背景图片上传
- 📱 Mobile responsive layout<br>移动端自适应布局
- 🎭 Multi-skin architecture — 5 built-in skins<br>多皮肤架构 — 5 款内置皮肤
